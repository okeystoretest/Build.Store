      ...
      <div className="flex gap-sm overflow-x-auto pb-2">
        {products.map((p) => (
          <button
            key={p.id}
            onClick={() => onSelect(p)}
            className="group flex w-40 shrink-0 flex-col rounded-md bg-surface-container-lowest p-sm text-left shadow-level-1 transition-colors hover:bg-primary-fixed/30"
          >
            <div className="flex h-24 w-full items-center justify-center overflow-hidden rounded-sm bg-surface-container">
              {p.imageUrl ? (
